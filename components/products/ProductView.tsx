...
const ProductView: FC<PropsI> = ({ product }) => {
  ...
  return (
    <div tw="md:flex md:items-center mt-8 md:mx-10 md:px-5">
      <div tw="w-full sm:ml-1  h-64 md:w-1/2 lg:h-96">
        <img
      ...
      <div tw="w-full max-w-lg mx-auto mt-5 md:ml-8 md:mt-0 md:w-1/2">
        <h3 tw="text-gray-700 uppercase text-lg ml-7">{name}</h3>
        <span tw="text-gray-500 mt-3 ml-7">${price}</span>

        <hr tw="my-3" />
        <p tw="text-gray-500 ml-2 mr-2">{description}</p>
        <hr tw="my-3" />
        <div tw="mt-2">
          <label tw="flex justify-center text-gray-700 text-sm" htmlFor="count">
            <span>Count:</span>
          </label>
